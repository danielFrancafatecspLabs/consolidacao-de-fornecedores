...
def get_all_fornecedores():
    from .db import db
    from .parser import FORNECEDOR_MAP, normalize_string
    fornecedores = list(db.get_collection('fornecedores').find({}, {'_id': 0}))
    agrupados = {}
    for f in fornecedores:
        nome_norm = normalize_string(f.get('fornecedor'))
        nome_principal = FORNECEDOR_MAP.get(nome_norm, f.get('fornecedor'))
